...
That re-raise runs only when the pass was itself driven by a signal trap. A teardown from the `EXIT` trap, or from the end of the read loop, has no re-raise of its own to replace. A signal recorded during one of those is left to the exit already under way.
