...
`run_mcp_server` installs `EXIT`, `INT`, `TERM`, `HUP` and `PIPE` traps, replacing any handler already in place. `EXIT` runs `_server_teardown`. The four signal traps run `_mcp_teardown_on_signal` with the signal's own name.
