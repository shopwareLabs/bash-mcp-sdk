...
- `code.primitives` =
  ...
  | Check a call's arguments against a schema | ad-hoc `jq` at the call site | `validate_tool_arguments` (`lib/mcpserver_core.sh`) | Enforces the whole `inputSchema` with diagnostics in precedence order missing > unknown > type > pattern > items > enum, and treats a jq failure as a rejection, never a skip. |
...
- `domain.terms` =
  ...
  - **diagnostic precedence** means the validator's fixed order: missing > unknown > type > pattern > items > enum.
- `docs.surfaces` =
